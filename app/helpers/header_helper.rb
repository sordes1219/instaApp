module HeaderHelper
  ...
  def display_icon
    return tag.div(class:'header-left') { |tag| link_to '←', :back } if request.fullpath.start_with?('/profiles') 
  ...
  def display_title
    return tag.div(class:'header-center') { |tag| @user.account } if request.fullpath.start_with?('/profiles')
